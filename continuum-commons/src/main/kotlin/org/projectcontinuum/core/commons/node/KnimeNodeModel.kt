...
import org.projectcontinuum.core.commons.utils.KnimeHelper
...
//            workflowDir.deleteRecursively()
      ...
      outputPorts.keys.forEachIndexed { index, portId ->
        ...
        if (outputFile.exists()) {
          nodeOutputWriter.createOutputPortWriter(portId).use { writer ->
            KnimeHelper.Companion.knimeContainerOutputTableToPortOutput(
              outputFile,
              writer
            )
          }
  ...
  fun prepareWorkflowInputs(
    ...
    return inputPorts.keys.mapIndexed { index, portId ->
      ...
      val knimeInputsTable = KnimeHelper.Companion.continuumTableToKnimeContainerInputTable(
        inputs[portId]!!
      )
